...
import de.untitledrpgwebapp.language.boundary.testfixture.LanguageDtoFixture;
...
class LanguageEndpointTest {
  ...
  private final DeploymentLanguageMapper mapper = mock(DeploymentLanguageMapper.class);
  ...
  void shouldCallFindAllLanguagesWithExpectedParameterAndReturnExpectedResultWhenFindAllIsCalled() {
    ...
    when(mapper.responsesToDtos(anyList())).thenReturn(LanguageDtoFixture.DTOS);
